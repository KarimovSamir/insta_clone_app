...
export class BlogController {
  ...
  getBlogList: RequestHandler = async (req: Request, res: Response) => {
    try {
      ...
      const { items, totalCount } = await this.blogService.findBlogs(queryInput);
  ...
  getBlogById: RequestHandler<{ id: string }> = async (req, res) => {
    try {
      const blog = await this.blogService.findBlogByIdOrFail(req.params.id);
  ...
  createBlog: RequestHandler<unknown, unknown, BlogCreateInput> = async (
    ...
    try {
      ...
      const createdBlog = await this.blogService.findBlogByIdOrFail(createdBlogId);
  ...
  updateBlogById: RequestHandler<{ id: string }, unknown, BlogUpdateInput> = async (
    req,
    res,
  ) => {
    try {
  ...
  createBlogPost: RequestHandler<{ id: string }, unknown, PostCreateByBlogInput> = async (
    req,
    res,
  ) => {
    try {
      const createdPostId = await this.postsService.createPostWithUrlBlogId(
        req.params.id,
        { ...req.body, blogId: req.params.id },
      );
      const createdPost = await this.postsService.findPostByIdOrFail(createdPostId);
  ...
  getBlogPostList: RequestHandler<{ id: string }> = async (req, res) => {
    try {
      ...
      const queryInput = setDefaultSortAndPaginationIfNotExist(sanitizedQuery);
      const { items, totalCount } = await this.postsService.findPostsByBlog(
        queryInput,
        req.params.id,
      );
